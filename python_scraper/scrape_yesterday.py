#!/usr/bin/env python3
"""
MOTUS LLC Trucking Yesterday Scraper
============================================================================
Paginates through all LLC carriers in MOTUS to find those updated yesterday
who belong to the trucking industry.

Filters applied:
1. Updated or Created date is exactly yesterday (UTC)
...
Output:
  leads_output.csv     — Yesterday's LLC Trucking leads
"""
...
def process_candidate(cand):
    ...
    try:
        ...
        # 1. Date filter: updateDate or createDate must be YESTERDAY
        ...
        is_yesterday = False
        if created_dt and created_dt.date() == YESTERDAY:
            is_yesterday = True
        if updated_dt and updated_dt.date() == YESTERDAY:
            is_yesterday = True
            
        if not is_yesterday:
            return {"dot": dot, "status": "skip", "reason": "Not updated yesterday"}
...
def main():
    print("=" * 65)
    print(f"MOTUS LLC Yesterday Trucking Scraper")
    print(f"Target update date (UTC): {yesterday_str}")
