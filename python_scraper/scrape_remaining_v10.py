#!/usr/bin/env python3
"""
MOTUS Scraper - High Speed Async Gap Scraper (v10)
============================================================================
Scrapes USDOT numbers starting from 4,582,560.
Strictly captures ONLY carriers that are NOT previously added into our database.
Upserts newly discovered carriers directly into Supabase 'carriers' table.
Appends each new lead in real time to unadded_leads_from_4582560_v10.csv.
Checkpoints state after each chunk to motus_scraper_state_v10.json.
"""
...
DEFAULT_CHUNK_SIZE  = 5000         # Scan in chunks of 5,000 DOTs
DEFAULT_CONCURRENCY = 300          # 300 concurrent async HTTP connections
...
def load_existing_dots_from_db(client, start_dot, end_dot):
    """
    Pulls existing usdot_number_num from Supabase carriers table for [start_dot, end_dot]
    so we don't send useless HTTP requests to MOTUS for carriers already in DB.
    """
    if not client:
        return set()
    
    print(f"[*] Pre-loading existing carriers from Supabase ({start_dot:,} to {end_dot:,})...", flush=True)
    existing = set()
    page_size = 1000
    offset = 0
    t0 = time.time()
    try:
        while True:
            ...
            if len(rows) < page_size:
                break
            offset += page_size
        print(f"[+] Loaded {len(existing):,} existing carriers from Supabase in {time.time() - t0:.1f}s", flush=True)
    except Exception as e:
        print(f"[!] Warning pre-loading Supabase DOTs: {e}", flush=True)
    return existing

def check_dots_in_supabase(client, dots_list):
    """
    Given a list of integer dots, queries Supabase using in_ filter to verify existence.
...
async def async_main(args):
    ...
    state_file  = args.state_file or STATE_FILE

    sep = "=" * 72
    print(sep, flush=True)
    print("MOTUS HIGH-SPEED ASYNC GAP SCRAPER v10 (UNADDED TO DATABASE ONLY)", flush=True)
    ...
    # 1. Load known dots from local files and Supabase
    known_dots = load_known_dots()
    print(f"[+] Loaded {len(known_dots):,} previously recorded USDOTs from CSVs to skip probing", flush=True)

    if client:
        db_dots = load_existing_dots_from_db(client, start_dot, end_dot)
        known_dots.update(db_dots)
        print(f"[+] Total known USDOTs in DB/CSVs to skip probing: {len(known_dots):,}", flush=True)

    # 2. Check if resuming from state file (unless --fresh is passed)
    resume_dot = start_dot
    total_previously_probed = 0
    total_previously_added = 0
        ...
        try:
            with open(state_file, "r", encoding="utf-8") as sf:
                ...
                if "current_dot" in state_data and state_data["current_dot"] > start_dot:
                    ...
                    total_previously_added = state_data.get("total_new_added", 0)
    ...
    async with aiohttp.ClientSession(headers=HEADERS, connector=connector) as session:
        while chunk_start <= end_dot:
            ...
            print(
                ...
                f"New leads captured: {found_new} | Speed: {rate:,.0f} DOTs/min | Elapsed: {elapsed:.0f}s",
                flush=True,
            )
            ...
            for future in asyncio.as_completed(tasks):
                ...
                if probed_count % 5000 == 0:
                    ...
                    print(f"  --> Probed {probed_count:,} DOTs in session | Found in chunk: {len(carriers_found_in_chunk)} | Speed: {curr_rate:,.0f} DOTs/min", flush=True)

            # Check found carriers against Supabase to strictly guarantee they aren't in DB
            if carriers_found_in_chunk:
                ...
                for dot, carrier in carriers_found_in_chunk:
                    ...
                    # This carrier is NOT in database! Add to CSV and upsert to Supabase
                    found_new += 1
            ...
            try:
                with open(state_file, "w", encoding="utf-8") as sf:
                    json.dump({
                        ...
                        "total_new_added": total_previously_added + found_new,
                        "session_new_added": found_new,
                        "start_dot": start_dot,
                        "end_dot": end_dot,
    ...
    print("\n" + sep, flush=True)
    print("FINISHED GAP SCAN v10", flush=True)
    print(sep, flush=True)
    print(f"  Total Probed (Session): {probed_count:,}")
    print(f"  New Leads Captured    : {found_new:,}")
    print(f"  Output CSV            : {output_csv}")
    print(sep, flush=True)

def parse_args():
    parser = argparse.ArgumentParser(description="MOTUS Scraper v10 - Scan USDOT gap starting from 4582560")
    ...
    parser.add_argument("--concurrency", type=int, default=DEFAULT_CONCURRENCY, help=f"Concurrent HTTP connections (default: {DEFAULT_CONCURRENCY})")
    parser.add_argument("--output", type=str, default=OUTPUT_CSV, help="Output CSV path")
    parser.add_argument("--state-file", type=str, default=STATE_FILE, help="State JSON path")
