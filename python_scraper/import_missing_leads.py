#!/usr/bin/env python3
"""
Imports leads_missing_from_4582560.csv (produced by scrape_missing_from_4582561.py)
into the Supabase `carriers` table. Same upsert-by-usdot_number pattern as
import_csv_to_supabase.py, but with its own source/progress files so it
doesn't touch that script's unrelated progress state.
"""
...
CSV_FILE = os.path.join(WORKSPACE_ROOT, "leads_missing_from_4582560.csv")
PROGRESS_FILE = os.path.join(WORKSPACE_ROOT, "import_missing_leads_progress.json")
BATCH_SIZE = 500
CONCURRENCY = 10
...
async def async_main():
    ...
    print("IMPORT MISSING LEADS -> SUPABASE (ZERO DUPLICATES GUARANTEE)")
