...
============================================================================
Scrapes USDOT numbers from 4,582,561 to 10,000,000 into a new CSV file.
...
CONCURRENCY = 400          # Concurrent async HTTP requests
OUTPUT_CSV  = "unadded_leads_from_4582561.csv"
...
def get_existing_dots():
    print("[*] Local CSV deduplication disabled. Probing all DOTs in range...", flush=True)
    return set()
...
async def async_main():
    ...
    print("MOTUS SCRAPER (USDOT 4,582,561 -> 10,000,000)", flush=True)
    ...
    existing_dots = get_existing_dots()

    # Load existing DOTs from OUTPUT_CSV if resuming
    file_dots = 0
                ...
                for row in reader:
                    ...
                    if dot_val:
                        try:
                            ...
                            pass
            print(f"[+] Resume mode: loaded {file_dots:,} unique DOTs from {OUTPUT_CSV}", flush=True)
        except Exception as e:
            print(f"[!] Error reading existing {OUTPUT_CSV}: {e}", flush=True)
    ...
    print("\n" + sep, flush=True)
    print("FINISHED SCRAPING RANGE", flush=True)
    print(sep, flush=True)
